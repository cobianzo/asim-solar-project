...
const rectangleRotationInteractionSetup = function () {
	...
	window.cocoDrawingRectangle.polygon.addListener(
		'mousedown',
		function (this: google.maps.Polygon, event: google.maps.MapMouseEvent) {
			...
			window.cocoDrawingRectangle.rotatingRectangleStartingVertexPoints = convertPolygonPathToPoints(
			...
			map.addListener('mousemove', function (event: google.maps.MapMouseEvent) {
				...
				if (
					!point ||
					!window.cocoDrawingRectangle.polygon ||
					!window.cocoDrawingRectangle?.rotatingRectangleStartingPoint
				)
					return;

				const [deltaX, deltaY] = [
					point.x - window.cocoDrawingRectangle.rotatingRectangleStartingPoint.x,
					point.y - window.cocoDrawingRectangle.rotatingRectangleStartingPoint.y,
				];
				...
				window.cocoDrawingRectangle.tempRotatedPoints = rotateRectangle(
					window.cocoDrawingRectangle.rotatingRectangleStartingVertexPoints,
					window.cocoDrawingRectangle?.polygonCenterPoint,
					deltaDegrees
				);
				if (window.cocoDrawingRectangle.tempRotatedPoints) {
					window.cocoDrawingRectangle.tempRotatedCoords = convertPointsArrayToLatLngString(
						map,
						window.cocoDrawingRectangle.tempRotatedPoints
					);
					if (window.cocoDrawingRectangle.tempRotatedCoords) {
						const latLngCoords = convertStringLatLngToArrayLatLng(
							window.cocoDrawingRectangle.tempRotatedCoords
						);
			...
			map.addListener('mouseup', function (event: google.maps.MapMouseEvent) {
				...
				window.cocoDrawingRectangle.currentInclinationAfterRotation = getInclinationByPolygonPath(
					window.cocoDrawingRectangle.polygon
				);
				...
				if (input) {
					const pathInString = window.cocoDrawingRectangle.polygon
						? (convertPolygonPathToStringLatLng(window.cocoDrawingRectangle.polygon) ?? '')
						: '';
