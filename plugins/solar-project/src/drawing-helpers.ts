...
export const paintBoundingBoxAsPolygon = (
	...
	const boundingBoxCoords =
		`${boundingBox.sw.latitude},${boundingBox.sw.longitude} ` +
...
export const paintSegment = function (
	gmap: google.maps.Map,
	stringCoords: string,
	segmentData: RoofSegmentStats,
	...
	window
		.paintAMarker(gmap, pol.getPath().getArray()[0], `${window.cocoAssetsDir}${'pixel.png'}`, MARKER_DOT)
...
export const removeRectangleInMap = (gmap: google.maps.Map, clearDrawingInfo = false) => {
	if (window.cocoDrawingRectangle?.polygon) {
		...
		if (clearDrawingInfo) window.cocoDrawingRectangle = {};
	}
};
...
export const paintCenterOfUsersRectangleInMap = (gmap: google.maps.Map) => {
	...
	console.log(
		'center rectangle painted by the user is ',
		polygonCenterCoords?.lat(),
		polygonCenterCoords?.lng()
	);
	const markerOptions = { ...MARKER_DOT, style: { ...MARKER_DOT.style, 'border-color': 'darkturquoise' } };
	if (polygonCenterCoords) {
		window
			.paintAMarker(gmap, polygonCenterCoords, `${window.cocoAssetsDir}${'pixel.png'}`, markerOptions)
			.then((marker) =>
				addAssociatedMarker(marker, window.cocoDrawingRectangle as AssociatedMarkersParent)
			);
	}
};
