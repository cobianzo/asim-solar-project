...
window.debug.showAllJSGlobalVarsInPopup = function (e: Event) {
	...
	if (window.cocoMaps) {
		...
		const cocoMapsVars = cocoMapsKeys
			.map((key) => {
	...
	if (window.cocoVars) {
		...
		innerHTML +=
			'<pre>' +
			showVariableAsString(
				'window.cocoVars (object from coco-gf-map=field plugin)',
				window['cocoVars'],
				new Set()
			) +
			' </pre>';
	...
	const globalCocoVars = Object.keys(window).filter(
		(key) =>
			(key.startsWith('coco') || key.startsWith('step')) &&
			!['cocoMaps', 'cocoVars', 'cocoDrawingRectangle'].includes(key)
	);
	...
	innerHTML +=
		'<p>variable with infor about the selection of a segment and creation of the rectangle<p><pre><ul>';
